...
# Proxy untuk memudahkan kontrol input (Keyboard + Gesture)
class InputProxy:
...
def main():
    ...
    # 2. Inisialisasi Komponen Core
    player = Player(500, 450)
    camera = Camera(WIDTH, HEIGHT, 2560, 1440) # Ukuran map Sanctuary
    dialogue = DialogueBox()
    currency = CurrencyManager()
    hud = HUD(currency)
    effects = VisualEffects(WIDTH, HEIGHT)
    
    # Logika Utama Game (Sistem Map & NPC)
    game_logic = SanctuaryLogic(dialogue)
    # Sesuaikan ukuran box dialog untuk 720p
    ...
    while True:
        ...
        if wardrobe_active:
            ...
            # Rendering while in wardrobe (keep background visible)
            ...
            # Left: Character Preview Area Frame
            ...
            preview_row = 4 # Facing Down, walking
            ...
            for idx, (cat_name, opt_name) in enumerate(categories):
                ...
                if is_selected:
                    # Draw a bright glowing backplate
                ...
                # Render Options selector (with arrows)
                if is_selected:
                    opt_text = f"◀  {opt_name}  ▶"
                else:
                    opt_text = f"   {opt_name}   "
        ...
        if minigame_manager.in_minigame:
            ...
            # Kontrol Gestur di Minigame
            if current_g != "None":
                target_hx = int(h_pos[0] * WIDTH)
                target_hy = int(h_pos[1] * HEIGHT)
                # Linear interpolation for 60fps smoothing (glides like a mouse)
                lerp_factor = 0.40
        ...
        # Pemetaan Input (Keyboard & Gestur)
        ...
        effects.draw_flash(screen)
        dialogue.draw(screen)
        hud.draw(screen, player, 100) # Senter selalu penuh di demo
